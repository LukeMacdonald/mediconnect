  ...
  Future<void> getDoctors()async{
    ...
    if(role == "patient") {
      ...
      var response = await http.get(
          Uri.parse("${appointmentIP}search/appointment_doctors/${int.parse(id)}"),
          headers: {'Content-Type': 'application/json'});

      _doctors = json.decode(response.body) as List;

      for(int i = 0;i < _doctors.length;i++) {
        _messages.add(MessageData(null, int.parse(id), _doctors[i], DateTime.now(), "", false));
    ...
    return Scaffold(
        ...
        body:Column(mainAxisAlignment: MainAxisAlignment.center,
         ...
         Padding(
           ...
           child: Text("Available Doctors to Message",
              style: TextStyle(
                  fontSize: 30,
